	...
	<div class="btn-group">
		...
		<div class="btn-group" role="group">
			...
			<ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
				...
				<li role="presentation"><a role="menuitem" tabindex="-1" href="<?=base()?>torneo/partidos/listado/<?=$torneo?>/<?=$categoria->ID_CatTorn?>/<?=$grupo->ID_VueltaGpo.suffix()?>"><i class="fa fa-arrow-circle-right"></i>  <b><?=$grupo->DenomVG?></b></a></li>
				<?php } ?>
				<li role="presentation" class="divider"></li>
				<li role="presentation" class="dropdown-header">Acciones para este grupo</li>
				<li role="presentation"><a role="menuitem" tabindex="-1" href="#" class="btn-jornada-crear"><i class="fa fa-calendar"></i> Crear jornada</a></li>
				<li role="presentation"><a role="menuitem" tabindex="-1" href="<?=base()?>torneo/partidos/grupo_formulario/<?=$torneo?>/<?=$categoria->ID_CatTorn?>/<?=$grupo_actual->ID_VueltaGpo.suffix()?>"><i class="fa fa-pencil"></i> Editar</a></li>
				<li role="presentation"><a role="menuitem" tabindex="-1" href="#" class="btn-eliminar" data-tabla="vueltas_gpos" data-id="<?=$grupo_actual->ID_VueltaGpo?>" data-location="<?=base()?>torneo/partidos/listado/<?=$torneo?>/<?=$categoria->ID_CatTorn.suffix()?>"><i class="fa fa-times"></i> Eliminar</a></li>
				<li role="presentation"><a role="menuitem" tabindex="-1" href="<?=base()?>torneo/jugadores/goleadores/<?=$torneo?>/<?=$categoria->ID_CatTorn.suffix()?>" class="btn-"><i class="fa fa-futbol-o"></i> Lista de goleadores</a></li>
		...
		<div class="panel-body padding0">
			...
			<?php
				foreach($jornada->partidos as $partido)
				{
					...
					{
						$editable = false;
						$puntaje1 = "({$partido->equipos[0]['puntaje']})";
						$puntaje2 = "({$partido->equipos[1]['puntaje']})";
						...
						if($partido->ganoSO == $partido->equipos[0]['id']) $ganador1 = '<i class="fa fa-trophy marginRight10" title="Ganador por shoot outs"></i> <small>(S.O.)</small>';
						if($partido->ganoSO == $partido->equipos[1]['id']) $ganador2 = '<i class="fa fa-trophy marginLeft10" title="Ganador por shoot outs"></i> <small>(S.O.)</small>';
					}
					
					$href = base() . "torneo/partidos/puntaje/{$torneo}/{$categoria->ID_CatTorn}/{$partido->ID_Partido}" . suffix();
			?>
				<tr>
					<td>					
						<div class="row marginLeft0">
							<div class="col-xs-5 text-right">
								<div class="row">
									<div class="col-xs-10 padding0 partidoPuntaje"><?=$ganador1?> <a href="<?=$href?>" class="text-primary" title="Establecer puntaje"><?=$partido->equipos[0]['nombre']?></a></div>
							...
							<div class="col-xs-5">
								<div class="row">
									...
									<div class="col-xs-10 padding0 partidoPuntaje"><a href="<?=$href?>" class="text-primary" title="Establecer puntaje"><?=$partido->equipos[1]['nombre']?></a> <?=$ganador2?></div>
								</div>
						</div>
					</td>
		...
		<form method="post" id="partidoForm" class="modal-content">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="alert alert-info form-wait marginTop20"><i class="fa fa-spinner fa-spin"></i> Registrando información...</div>
					<div class="alert alert-success form-success marginTop20"><i class="fa fa-check"></i> El registro se realizó exitosamente.</div>
					<?php } ?>
				</div>
				<div class="modal-footer form-footer-cerrar">
					<button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-times"></i> Cerrar</button>
