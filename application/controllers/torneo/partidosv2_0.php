...
class Partidos extends CI_Controller {
	...
	public function puntaje($torneo, $categoria, $id)
	{
		if($this->moguardia->isloged(true) and $torneo > 0 and $categoria > 0 and $id > 0)
		{
			...
			$t->id($torneo);
			$t->grupo_categoria($categoria);
			$grupo = $t->grupos(true, true);
			$grupo = $grupo[0];
			
			//Identificar partido
			foreach($grupo->jornadas as $jornada)
			{
			...
			{
				$i['partido'] = $partido;
				$i['grupo'] = (object) array('id' => $grupo->ID_VueltaGpo, 'nombre' => $grupo->DenomVG);
