...
class Equipos extends CI_Controller {
	...
	public function importar($torneo, $categoria)
	{
		if($this->moguardia->isloged(true) and $torneo > 0 and $categoria > 0)
		{
			...
			$torneos->generar();
			if(count($torneos->registros) > 0)
			{
				foreach($torneos->registros as $torneoInfo)
				{
					if($torneoInfo->ID_Torneo != $torneo) //No contar este torneo
					{
						//Crear objeto de torneo y definir la categoría para obtener sus equipos
						$e = $this->moequipo->init();
						$e->torneo($torneoInfo->ID_Torneo);
						$e->categoria($categoria);
						$equipos = $e->torneos_equipos();
						$torneoInfo->equipos = $e->torneos_equipos();
						if(count($equipos) > 0) $i['torneos'][] = $torneoInfo;
					}
				}
			}
