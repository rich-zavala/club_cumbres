...
class Moequipo extends CI_Model
{
	...
	public function jugadores_insert($i)
	{
		if($this->id > 0)
		{
			...
			$q = $this->db->select('NomJug')->where('ID_Equipo', $this->id)->get('equipos_jugadores');
			if($q->num_rows() > 0)
			{
				foreach($q->result() as $r)
				{
					$pos = array_search($r->NomJug, $this->jugadores_insert);
	...
	public function torneos_equipos()
	{
		...
		if($q->num_rows() > 0)
		{
			foreach($q->result() as $torneo)
			{
				...
				if($q->num_rows() > 0)
				{
					foreach($q->result() as $r) $equipos[] = $r;
					
					$torneos[] = array(
						'nombre' => $torneo->NomTorneo,
						'equipos' => $equipos
					);
				}
			}
	...
	public function goleadores($limit)
	{
		$r = array();
